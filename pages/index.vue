...
const initFlipPageTransition = (e: Event) => {
  ...
  const flipElement = target.hasAttribute('data-flip-id') ? target : target.closest('[data-flip-wrapper]')?.querySelector('[data-flip-id]')
...
definePageMeta({
  pageTransition: {
    ...
    onLeave: (el, done) => {
      ...
      const { transitioning, setPageTransitionWatcherCallback } = usePageTransition()
...
<template>
  <main>
    <section class="relative min-h-screen flex flex-col items-center overflow-x-clip">
      <div class="z-[1] absolute bottom-[1rem] md:bottom-[7rem] flex flex-col items-end">
        <h1 class="text-[10rem] md:text-[40rem] leading-[4rem] md:leading-[10rem] italic text-primary-200">
          &nbsp; Théo <br />
          Dupont
        </h1>
        <h6 class="-translate-x-4 absolute top-2/3 md:top-0 left-1/3 md:left-2/3">[Développeur Front End] (Graphiste)</h6>
      </div>

      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/3 aspect-square bg-neutral-900 rounded-full blur-3xl">
        <div class="absolute top-1/3 right-0 -translate-x-1/2 -translate-y-1/2 w-[10rem] aspect-square bg-secondary-100 rounded-full blur-3xl"></div>
      </div>
    </section>
    ...
    <section class="relative mt-52 mb-[20vh] min-h-screen overflow-x-clip">
      ...
      <div class="position sticky px-10 md:px-52 w-full min-h-screen flex items-center justify-end pointer-events-none">
    ...
    <section id="contact" class="relative min-h-screen flex flex-col">
      ...
      <div data-flip-wrapper="1" data-animate-element="contact" class="sticky top-0 w-full h-screen pb-16 flex flex-col justify-between bg-background">
        <div class="mt-10 md:mt-0 p-4 md:p-0 flex flex-col md:flex-row items-center gap-16 md:gap-0">
          <h2 class="text-3xl md:text-5xl md:px-40 md:w-1/2 italic">Je suis disponible pour vos idées. Prenons contact !</h2>
        ...
        <div class="px-4 flex justify-center">
          <div class="flex flex-col md:items-end">
            ...
            <p class="md:w-[20rem]">
              Hi there! I’m a freelance designer & developer from Canada with a love for all things web. I’m currently working with clients & collaborating with
              agencies worldwide.
            </p>
          </div>
        </div>
      ...
      <div data-animate-element="contact" class="sticky top-0 w-full h-screen pb-16 flex flex-col justify-between bg-background">
        <div class="mt-10 md:mt-0 p-4 md:p-0 flex flex-col md:flex-row items-center gap-16 md:gap-0">
          <h2 class="text-3xl md:text-5xl md:px-40 md:w-1/2 italic">Je suis disponible pour vos idées. Prenons contact !</h2>
          <div class="md:w-1/2 flex items-end md:justify-center gap-3">
            <div data-flip-id="1" class="w-1/3 aspect-[9/13] bg-surface">
      ...
      <div data-animate-element="contact" class="sticky top-0 w-full h-screen pb-16 flex flex-col justify-between bg-background text-primary-200">
        <div class="-z-[1] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/2 aspect-square bg-neutral-900 rounded-full blur-3xl"></div>

        <div class="mt-10 md:mt-0 p-4 md:p-0 flex flex-col md:flex-row items-center gap-16 md:gap-0">
          <h2 class="text-3xl md:text-5xl md:px-40 md:w-1/2 italic">Je suis impatient de travailler avec vous.</h2>
          <div class="md:w-1/2 flex items-end md:justify-center gap-3">
            <div data-flip-id="1" class="w-1/3 aspect-[9/13] bg-surface">
              <img
      ...
      <div data-animate-element="contact" class="sticky top-0 h-screen w-full pointer-events-none"></div>
    </section>
